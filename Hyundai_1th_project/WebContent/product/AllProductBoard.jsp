		...
		<div class="hs-row box" style="height: 100px;">
			<ol class="breadcrumb col-r">
				...
				<li class="breadcrumb-item"><a class="breadcrumb-item-a"
					href="BoardServlet?command=product&index=1">All</a></li>
			</ol>
		</div>
		<h2 class="hs-row tit_page tit-bg-none">
			<span><em class="f_jost">All</em></span>
		</h2>
				...
				<div class="dropdown-menu">
					<ul class="dropdown-menu-ul">
						<li><a href="BoardServlet?command=product&index=1"
							class="dropdown-item on" id="brand_All"> <span>ALL</span>
						</a></li>
				...
				<div class="list-sort-area">
					...
					<c:set var="index" value="${index}"></c:set>
						<c:if test="${index eq '1'}">
							<a href="BoardServlet?command=product&index=1" class="active">최근등록순</a> 
							<a href="BoardServlet?command=product&index=2">많이팔린순</a>
							<a href="BoardServlet?command=product&index=3">낮은가격순</a> 
							<a href="BoardServlet?command=product&index=4">높은가격순</a>
						</c:if>
						<c:if test="${index eq '2'}">
							<a href="BoardServlet?command=product&index=1">최근등록순</a> 
							<a href="BoardServlet?command=product&index=2" class="active">많이팔린순</a>
							<a href="BoardServlet?command=product&index=3">낮은가격순</a> 
							<a href="BoardServlet?command=product&index=4">높은가격순</a>
						</c:if>
						<c:if test="${index eq '3'}">
							<a href="BoardServlet?command=product&index=1">최근등록순</a> 
							<a href="BoardServlet?command=product&index=2">많이팔린순</a>
							<a href="BoardServlet?command=product&index=3" class="active">낮은가격순</a> 
							<a href="BoardServlet?command=product&index=4">높은가격순</a>
						</c:if>
						<c:if test="${index eq '4'}">
							...
							<a href="BoardServlet?command=product&index=2">많이팔린순</a>
							<a href="BoardServlet?command=product&index=3">낮은가격순</a> 
							<a href="BoardServlet?command=product&index=4" class="active">높은가격순</a>
						</c:if>
					</div>
				</div>
